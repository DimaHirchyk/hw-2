...
const costAllSites = companyWebsites
    .map((el) => el.siteCost)
    .reduce((acc, site) => acc + site, 0);
document.body.innerHTML += `1) загальна вартість усіх сайтів - ${costAllSites} 
    <br />
`;
const siteMadeBetween = companyWebsites.filter((el) => el.releaseYear > 2000 && el.releaseYear < 2009).length;
...
const siteSponsorshipInvestments = companyWebsites
    .map(({ sponsors }) => sponsors.reduce((acc, { investment }) => (acc += investment), 0))
...
document.body.innerHTML += `4) створити загальний список усіх спонсорів (поки можуть повторюватись, просто зібрати усі у масив)
 - ${generalListAllSponsors} 
    <br />
`;
const yearWhenProfitsWereHighest = Math.max(...companyWebsites.map((el) => el.surplus));
const year = (_a = companyWebsites.find((site) => site.surplus === yearWhenProfitsWereHighest)) === null || _a === void 0 ? void 0 : _a.releaseYear;
...
document.body.innerHTML += `6) упорядкувати список за спаданням прибутку
 - ${listDescendingOrderProfit} 
    <br />
`;
const copyObjectCostLess10000 = JSON.parse(JSON.stringify(companyWebsites.filter((el) => el.siteCost < 100000)));
const copyObjectCostMore10000 = JSON.parse(JSON.stringify(companyWebsites.filter((el) => el.siteCost > 100000)));
